...
- (UIImage *)deleteBackgroundOfImage:(UIImageView *)image
{
    ...
    {
         CGContextClipToMask(UIGraphicsGetCurrentContext(), rect, mask.CGImage);//进行裁剪
        [image.image drawAtPoint:CGPointZero];//将原图放入当前画布
    ...
    croppedRect.origin.y = rect.size.height - CGRectGetMaxY(aPath.bounds);//This because mask become inverse of the actual image;当前坐标原点在左下方
