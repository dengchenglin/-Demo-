...
@interface PieChartView()<RenderViewDataSource,RenderViewtDelegate>
...
- (id)initWithFrame:(CGRect)frame withValue:(NSMutableArray *)valueArr withColor:(NSMutableArray *)colorArr
{
    ...
    if (self) {
        ...
        self.centerView.frame = CGRectMake((frame.size.width - 80)/2, (frame.size.height - 80)/2, 80, 80);
        self.centerView.backgroundColor =[UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
        
        [self.centerView.layer setMasksToBounds:YES];
        [self.centerView.layer setCornerRadius:40];
        
        
        int titleWidth = 65;
        self.title = [[UILabel alloc]initWithFrame:CGRectMake((80 - titleWidth)/2,15 , titleWidth, 17)];
        ...
        int amountWidth = 75;
        self.amountLabel = [[UILabel alloc]initWithFrame:CGRectMake((80 - amountWidth)/2, 40, amountWidth, 22)];
