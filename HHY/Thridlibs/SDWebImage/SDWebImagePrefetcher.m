...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", (int)total - (int)_skippedCount, (int)_skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    NSLog(@"Prefetched %d out of %lu", (int)_finishedCount, (unsigned long)[self.prefetchURLs count]);
