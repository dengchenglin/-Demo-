...
- (void)viewDidLoad
{
    ...
    for (NSInteger i = 0;i<8;i++)
    {
        ...
        btn.frame = CGRectMake(15+(i%3)*100, 64+(i/3)*100, 90, 90);
        btn.tag = i;
        btn.titleLabel.font = [UIFont boldSystemFontOfSize:20];
        [btn setImage:[UIImage imageNamed:@"state1.png"] forState:UIControlStateNormal];
        ...
        [btn addTarget:self action:@selector(butarr:) forControlEvents:UIControlEventTouchUpInside];
        [self.itemArray addObject:btn];

    }
}
-(void)butarr:(UIButton *)but
{
    NSLog(@"%d",but.tag);
}
